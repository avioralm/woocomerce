<?php
add_action('wp_enqueue_scripts', 'store_max_styles',199);
function store_max_styles(){
    $themeVersion = wp_get_theme()->get('Version');
    wp_enqueue_style( 'store-max-style',
        get_stylesheet_uri(),
        array( 'th-shop-mania-style' ),
        $themeVersion // This only works if you have Version defined in the style header.     
    );
    wp_add_inline_style('store-max-style', store_max_custom_style());
}
/**********************/
//customize setting
/**********************/

function store_max_setting( $wp_customize ){
    // theme color
...
$wp_customize->add_control( 
    new WP_Customize_Color_Control($wp_customize,'th_shop_mania_theme_clr', array(
        'label'      => __('Theme Color', 'store-max' ),
...
add_action( 'customize_register', 'store_max_setting', 100 );
...
//custom style
/***************************/
function store_max_custom_style(){
$th_shop_mania_theme_clr = esc_html(get_theme_mod('th_shop_mania_theme_clr','#005904'));
$th_shop_mania_style = '';
